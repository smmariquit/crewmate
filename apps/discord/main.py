...
class ProjectManager:
    def __init__(self):
        ...
        self.shop_items = {}  # Empty shop by default
...
async def help_command(ctx):
    ...
    embed.add_field(
        name="📋 **BASIC COMMANDS**",
        value="Essential commands for getting started:\n• `!commands` - Shows this help message\n• `!roles` - Shows available roles\n• `!nickname <name> <profession>` - Changes your nickname",
        inline=False
    )
    ...
    embed.add_field(
        ...
        value="Commands for viewing and exploring projects:\n• `!projects` - Shows all ongoing projects\n• `!project <name> dashboard` - Shows project dashboard\n• `!project <name> details` - Shows detailed project info",
        inline=False
    )
    ...
    embed.add_field(
        ...
        value="Commands for joining and working on projects:\n• `!project <name> assign` - Assign yourself to tasks (interactive)\n• `!completed project <name>` - Mark tasks as completed (interactive)",
        inline=False
    )
    
    # Project Management (Admin/Team Leader)
    embed.add_field(
        name="⚙️ **PROJECT MANAGEMENT** *(Admin/Team Leader)*",
        value="Commands for creating and managing projects:\n• `!new project` - Creates a new project (interactive)\n• `!edit project <name>` - Edits an existing project (interactive)\n• `!delete project <name>` - Deletes a project (interactive)",
        inline=False
    )
    ...
    embed.add_field(
        name="🏆 **REWARDS & SHOP**",
        value="Commands for managing points and purchasing items:\n• `!rewards` - Shows your current points and tasks\n• `!shop` - Shows available items (starts empty)\n• `!shop add <item> <price>` - Adds items to shop (Admin only)",
        inline=False
    )
    ...
    embed.add_field(
        ...
        value="Commands for configuring the bot system:\n• `!forums <channel>` - Configures forum channel\n• `!perm add/remove/view <role>` - Manages role permissions (Owner only)",
        inline=False
    )
...
async def show_shop(ctx):
    ...
    try:
        ...
        if not pm.shop_items:
            embed.add_field(
                name="No Items Available",
                value="The shop is currently empty. Items will be added by administrators.",
                inline=False
            )
            embed.add_field(
                name="How to add items",
                value="Administrators can add items using `!shop add <item> <price>`",
                inline=False
            )
        else:
            embed.add_field(
...
async def edit_project_interactive(user, project_name):
    ...
    try:
        ...
        # Update forum post to reflect changes
        forum_updated = await update_forum_post(project_name, project)
        
        # Send confirmation message
        embed = discord.Embed(
            title="✅ Project Updated Successfully!",
            description=f"Project **{project_name}** has been updated.",
            color=0x00ff00
        )
        embed.add_field(name="Project Data", value="✅ Updated in database", inline=True)
        
        if forum_updated:
            embed.add_field(name="Forum Post", value="✅ Updated in forum", inline=True)
        else:
            embed.add_field(name="Forum Post", value="⚠️ Could not update (may not exist)", inline=True)
...
async def update_forum_post(project_name, project):
    ...
    try:
        ...
        # Try to find and edit the original post (first message in thread)
        try:
            # Get the first message in the thread (the original post)
            async for message in thread.history(limit=1, oldest_first=True):
                if message.author == bot.user:  # Make sure it's our bot's message
                    await message.edit(embed=embed)
                    print(f"Successfully updated original forum post for project: {project_name}")
                    return True
                else:
                    # If the first message isn't from our bot, send a new update
                    await thread.send(embed=embed)
                    print(f"Sent new update message for project: {project_name}")
                    return True
        except Exception as e:
            print(f"Error editing original post for project {project_name}: {str(e)}")
            # Fallback: send a new message if editing fails
            await thread.send(embed=embed)
            print(f"Sent fallback update message for project: {project_name}")
            return True
